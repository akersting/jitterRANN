...
#'M) time. Ties are broken randomly.
#'
#'The \code{jitterRANN} package utilizes the Approximate Near Neighbor (ANN) C++
...
#'  larger point sets; in this package only "kd" is implemented; for "bd", use
#'  the original package \code{RANN} instead
#'@param searchtype See details; in this package only "standard" is implemented;
#'  for others, use the original package \code{RANN} instead
                ...
                searchtype=c("standard","priority","radius"),radius=0.0,eps=0.0)
{
  ...
  if (treetype != "kd")
    stop("Tie-breaking is only implemented for the 'kd' treetype. ",
         "For the 'bd' treetype please use the original RANN package.")

  if (searchtypeInt != 1)
    stop("Tie-breaking is only implemented for the 'standard' searchtype. ",
         "For the other searchtypes please use the original RANN package.")
  ...
  results <- .C("get_NN_2Set",
                ...
                nn.idx   = integer(k*NQ),
                nn       = double(k*NQ), PACKAGE="jitterRANN")
